...
impl CiLogsView {
    ...
    pub fn render(&mut self, frame: &mut Frame, app: &AppView) {
        ...
        let logs_loading = self.loading_issues.contains(&issue_key);
        ...
        let title_spans = if check_runs.is_empty() {
            ...
            let active_tab = self.clamp_active_tab(check_runs.len());
...
pub async fn update(app: &mut AppView, key_event: KeyEvent) {
    ...
    match key_event.code {
        ...
        KeyCode::Char('g') if previous_was_g => {
...
fn spawn_fix_ci(app: &mut AppView) {
    ...
    app.ci_log_popup.close();
    app.input_focus = crate::app::InputFocus::List;
    actions::fix_ci::spawn(app.message_tx.clone(), repo_path, head_branch, ci_error);
}
